...
from nirpredict.utils.loss import CombinedLoss
...
def build_model(device, ssim_weight=0.1, lr=0.005, outdir="output"):
    nir_model = NIRPredictor().to(device)

    criterion = CombinedLoss(ssim_weight=ssim_weight)
    optimizer = torch.optim.Adam(nir_model.parameters(), lr=lr)

    load_best_weights(nir_model, optimizer, outdir=outdir, device=device)
